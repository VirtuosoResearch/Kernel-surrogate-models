...
class CLIPModelOutput(AbstractModelOutput):
    ...
    def get_embeddings(
        ...
        """
        img_embs, txt_embs = (
            torch.zeros(size, embedding_dim).cuda(),
            torch.zeros(size, embedding_dim).cuda(),
        )
    ...
    def get_output(
        ...
        image_embeddings, text_embeddings, _ = torch.func.functional_call(
    ...
    def get_out_to_loss_grad(
        ...
        image_embeddings, text_embeddings, temp = torch.func.functional_call(
            model, (weights, buffers), args=(), kwargs=clip_inputs
        )
...
class ModularModelOutput(AbstractModelOutput):
    """Margin for text classification models. This assumes that the model takes
    in input_ids, token_type_ids, and attention_mask.
    ...
    def get_output(
        ...
        logits = model(x)[:, -1]
        bindex = torch.arange(logits.shape[0]).to(logits.device, non_blocking=False)
        logits_correct = logits[bindex, y]

        cloned_logits = logits.clone()
        cloned_logits[bindex, y] = torch.tensor(
            -torch.inf, device=logits.device, dtype=logits.dtype
        )
    ...
    def get_out_to_loss_grad(
        self, model, weights, buffers, batch,
    ) -> Tensor:
        #batch = batch.value
        batch = batch[0]
        x = batch[:, :-1]
...
TASK_TO_MODELOUT = {
    ...
    "modular_arithmetic": ModularModelOutput,
}
