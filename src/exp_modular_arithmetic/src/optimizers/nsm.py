...
class NSM(torch.optim.Optimizer):
    def __init__(self, params, base_optimizer, sigma=0.05, distribution=None, **kwargs):
        ...
        # print(f"Base optimizer: {self.base_optimizer}")
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if store_perturb:
                    ...
                    e_w = torch.randn(p.data.shape, device=p.device) * group["sigma"]
                    # print(f"Time taken to sample: {time()-start_time}, param shape: {p.data.shape}")
    ...
    @torch.no_grad()
    def first_head_step(self, zero_grad=False, store_perturb=True):
        ''' take a perturbation step of the original weights '''
        for group in self.param_groups:
            p = group["params"][-1]
            if not p.requires_grad: continue
