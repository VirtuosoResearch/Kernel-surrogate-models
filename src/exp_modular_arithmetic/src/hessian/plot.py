...
def hessian_plot(log, config, save_dir="./results/"):
    ...
    try:
        # Hessian bound
        plt.plot(log["train_hessian"]["iter"], log["train_hessian"]["value"], label="train_Hessian")
        plt.plot(log["valid_hessian"]["iter"], log["valid_hessian"]["value"], label="valid_Hessian")
        # Plot
        if train_converge > 0:
            plt.axvline(x=train_converge, color='blue', linestyle='--', linewidth=1, label='train convergence')
        if val_converge > 0:
        ...
        plt.grid()
        plt.annotate(time_str, xy=(0.2, 0.5), xycoords='axes fraction', fontsize=12, color='purple', ha='center')
        plt.savefig(f"{save_dir}compare/compare_{log.label}.png", dpi=150)
        plt.draw()
        plt.close()
    except Exception as e: 
        print(e)
