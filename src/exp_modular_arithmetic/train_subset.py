...
def assign_group(sample, bar=25) -> Tuple[int, int]:
    t = sample[0]
    x, y = t[0].item(), t[2].item()
    return (x // bar, y // bar)

def build_groups(dataset, bar) -> Dict[Tuple[int, int], List[int]]:
    groups = defaultdict(list)
    for idx, sample in enumerate(dataset):
        gid = assign_group(sample, bar=bar)
        groups[gid].append(idx)
    return dict(groups)

def save_groups(groups: Dict[Tuple[int, int], List[int]], path: str):
    dumpable = {json.dumps(list(k)): v for k, v in groups.items()}
    with open(path, "w") as f:
        json.dump(dumpable, f, indent=2)

def load_groups(path: str) -> Dict[Tuple[int, int], List[int]]:
    with open(path, "r") as f:
        raw = json.load(f)
    return {tuple(json.loads(k)): v for k, v in raw.items()}

def save_chosen_groups(chosen_groups_list: List[List[Tuple[int,int]]], path: str):
    dumpable = [[list(g) for g in one_pick] for one_pick in chosen_groups_list]
    with open(path, "w") as f:
        json.dump(dumpable, f, indent=2)

def load_chosen_groups(path: str) -> List[List[Tuple[int,int]]]:
    with open(path, "r") as f:
        raw = json.load(f)
    return [[tuple(g) for g in one_pick] for one_pick in raw]

def prepare_subsets_by_group_sampling(
    dataset,
    alpha: float = 0.5,
    m: int = 5,
    bar=25,
    seed: int = None,
    groups_path: str = "groups.json",
    chosen_path: str = "chosen_groups.json",
):
    ...
    if not (os.path.exists(groups_path) and os.path.exists(chosen_path)):
        print(f"Warning: Building groups and saving to {groups_path}")
        groups = build_groups(dataset, bar=bar)
        save_groups(groups, groups_path)

        group_ids = list(groups.keys())
        if len(group_ids) == 0:
            save_chosen_groups([], chosen_path)
            return [], [], groups

        k_groups = max(1, int(len(group_ids) * alpha))

        chosen_groups_list: List[List[Tuple[int,int]]] = []
        for _ in range(m):
            chosen = rng.sample(group_ids, k_groups)
            chosen_groups_list.append(chosen)

        save_chosen_groups(chosen_groups_list, chosen_path)

    groups = load_groups(groups_path)
    chosen_groups_list = load_chosen_groups(chosen_path)

    subsets = []
    train_ids_list = []
    for chosen in chosen_groups_list:
        indices = []
        for gid in chosen:
            if gid not in groups:
                continue
            indices.extend(groups[gid])
        subsets.append(Subset(dataset, indices))
        train_ids_list.append(indices)

    return subsets, train_ids_list, chosen_groups_list, groups
...
def run_experiment(config):
    ...
    test_loader = DataLoader(valid_dataset, batch_size=config.train.batch_size, shuffle=False, num_workers=8, pin_memory=torch.cuda.is_available())
    ...
    for model_id, train_loader in enumerate(train_loaders):
        print(f"Training on subset {model_id} with {len(train_loader.dataset)} samples")
        ...
        train_num = len(train_loader.dataset)
        valid_num = len(test_loader.dataset)
        phase1_folder = os.path.join(model_path, 'phase1')
        phase2_folder = os.path.join(model_path, 'phase2')
        phase3_folder = os.path.join(model_path, 'phase3')
        ...
        for e in pbar:
            ...
            data_loaders = [(train_loader, True), (test_loader, False)]
            ...
            if do_log:
                ...
                if config.train.hessian_log_every > 0:
                    ...
                    hessian_calculator = Hessian_Calculator(model=model, loss_fn=loss_fn, p=task.p, dataloader=train_loader, valid_dataloader=test_loader, device=device)
